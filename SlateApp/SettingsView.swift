    ...
    @ViewBuilder private var skillsTab: some View {
        ...
        Section("Local tools · MCP") {
            VStack(alignment: .leading, spacing: 5) {
                Label("Offline-only stdio", systemImage: "network.slash")
                    .font(.callout.weight(.semibold))
                Text("Slate accepts executable MCP servers only. Their child process is denied network access, and Slate asks before every tool call. HTTP and SSE servers are not supported.")
                    .font(.caption).foregroundStyle(.secondary)
                HStack(spacing: 6) {
                    Label("No network", systemImage: "checkmark.shield")
                    Label("Approval per call", systemImage: "hand.raised")
                    Label("Local audit", systemImage: "list.bullet.rectangle")
                }
                .font(.caption2.weight(.medium)).foregroundStyle(.secondary)
            }

            if model.localTools.servers.isEmpty {
                ContentUnavailableView("No local tools", systemImage: "wrench.and.screwdriver",
                                       description: Text("Add a trusted MCP server executable stored on this Mac."))
                    .frame(minHeight: 110)
            }

            ForEach(model.localTools.servers) { server in
                VStack(alignment: .leading, spacing: 8) {
                    HStack(spacing: 10) {
                        Toggle("", isOn: Binding(
                            get: { server.enabled },
                            set: { enabled in
                                var copy = server; copy.enabled = enabled
                                model.localTools.update(copy)
                            }))
                            .labelsHidden()
                        VStack(alignment: .leading, spacing: 1) {
                            Text(server.name).font(.callout.weight(.medium))
                            Text((server.executablePath as NSString).abbreviatingWithTildeInPath)
                                .font(.caption2.monospaced()).foregroundStyle(.secondary)
                                .lineLimit(1).truncationMode(.middle)
                        }
                        Spacer()
                        Text(server.enabled ? (model.localTools.statusByServer[server.id] ?? "Not scanned") : "Disabled")
                            .font(.caption2).foregroundStyle(.secondary)
                        Button(role: .destructive) { model.localTools.remove(server.id) } label: {
                            Image(systemName: "trash")
                        }
                        .buttonStyle(.borderless).help("Remove server")
                    }
                    TextField("Arguments (one per line)", text: Binding(
                        get: { server.arguments.joined(separator: "\n") },
                        set: { value in
                            var copy = server
                            copy.arguments = value.components(separatedBy: .newlines)
                    ...
                    TextField("Allowed working directory (required)", text: Binding(
                        get: { server.workingDirectory ?? "" },
                        set: { value in
                            var copy = server
                            let clean = value.trimmingCharacters(in: .whitespacesAndNewlines)
                            copy.workingDirectory = clean.isEmpty ? nil : clean
                            model.localTools.update(copy)
                        }))
                        .textFieldStyle(.roundedBorder).font(.caption.monospaced())
                }
            ...
            if !model.localTools.tools.isEmpty {
                DisclosureGroup("Discovered tools (\(model.localTools.tools.count))") {
                    VStack(alignment: .leading, spacing: 6) {
                        ForEach(model.localTools.tools, id: \.spec.name) { tool in
                            HStack(alignment: .top, spacing: 8) {
                                Image(systemName: "wrench.adjustable")
                                    .foregroundStyle(.secondary).frame(width: 16)
                                VStack(alignment: .leading, spacing: 1) {
                                    Text(tool.originalName).font(.caption.weight(.semibold))
                                    Text(tool.spec.description).font(.caption2).foregroundStyle(.secondary)
                                        .lineLimit(2)
                                }
                            }
                        }
            ...
            HStack {
                Button("Add executable…") {
                    guard model.requirePro(.localTools) else { return }
                    let panel = NSOpenPanel()
                    panel.title = "Choose a local MCP server"
                    panel.prompt = "Add Server"
                    panel.canChooseDirectories = false
                    panel.canChooseFiles = true
                    panel.allowsMultipleSelection = false
                    if panel.runModal() == .OK, let url = panel.url {
                        model.localTools.add(executable: url)
                        Task { await model.localTools.rescan(gate: model.coordinator) }
                    }
                }
                Button(model.localTools.isScanning ? "Scanning…" : "Rescan") {
                    guard model.requirePro(.localTools) else { return }
                    Task { await model.localTools.rescan(gate: model.coordinator) }
                }
                .disabled(model.localTools.isScanning || model.localTools.servers.isEmpty)
                Button("View audit…") { showAudit = true }
                    .disabled(model.localTools.tools.isEmpty)
                Spacer()
                Text("\(model.localTools.tools.count) available")
                    .font(.caption).foregroundStyle(.secondary)
            }
        }
